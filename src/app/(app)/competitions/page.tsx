  ...
  return <section className="mx-auto max-w-5xl px-4 pb-28 pt-6 sm:px-6 lg:px-10" aria-labelledby="competitions-heading">
    ...
    {loading ? <div className="flex min-h-56 items-center justify-center gap-2 text-sm text-gray-500"><Loader2 className="h-4 w-4 animate-spin" /> Loading competitions</div> : items.length ? <div className="mt-6 grid gap-4 md:grid-cols-2">{items.map((competition) => {
      ...
      return <article key={competition.id} className={`border p-5 ${active ? "border-accent-green bg-accent-green/5" : "border-bg-border bg-bg-soft"}`}>
        ...
        <button type="button" disabled={active || !selectable || full || joining === competition.id} onClick={() => void selectCompetition(competition.id)} className="mt-5 flex min-h-11 w-full items-center justify-center gap-2 bg-accent-green px-4 text-sm font-black text-black disabled:cursor-not-allowed disabled:bg-bg-elevated disabled:text-gray-500">{joining === competition.id ? <><Loader2 className="h-4 w-4 animate-spin" /> Joining</> : active ? "Current competition" : full ? <><Lock className="h-4 w-4" /> Competition full</> : selectable ? competition.joined ? "Switch to competition" : "Join competition" : <><Lock className="h-4 w-4" /> Enrollment closed</>}</button>
      </article>;
